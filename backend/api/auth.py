...
async def get_captcha():
    """获取CAPTCHA"""
    try:
        # 生成会话ID（简单实现，生产环境建议使用更安全的方式）
        import uuid
        session_id = str(uuid.uuid4())

        captcha_data = captcha_service.generate_captcha(session_id)
        ...
        logger.error(f"获取CAPTCHA失败: {str(e)}")
        raise HTTPException(
...
async def send_verification_code(
    ...
    try:
        # 验证CAPTCHA
        if not hasattr(request, 'captcha_id') or not hasattr(request, 'captcha_answer') or not hasattr(request, 'session_id'):
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail="缺少CAPTCHA验证信息"
            )

        if not captcha_service.verify_captcha(request.captcha_id, request.captcha_answer, request.session_id):
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail="CAPTCHA验证失败，请重新获取"
            )
